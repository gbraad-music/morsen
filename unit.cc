...
__unit_callback int8_t unit_init(const unit_runtime_desc_t *desc)
{
  ...
  // Initialize morse with modified descriptor for stereo output compatibility
  unit_runtime_desc_t morse_desc = *desc;
  morse_desc.output_channels = 2;
  return s_synth_instance.Init(&morse_desc);
}
...
__unit_callback void unit_render(const float *in, float *out, uint32_t frames)
{
  ...
  {
    float sample = stereo_buffer[i * 2];
